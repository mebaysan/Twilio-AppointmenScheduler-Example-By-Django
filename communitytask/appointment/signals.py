...
def schedule_appointment(sender, instance, **kwargs):
    if SCHEDULER.get_job(instance.get_scheduler_id):  # update the time of appointment
        SCHEDULER.remove_job(instance.get_scheduler_id)
    SCHEDULER.add_job(TwilioInterface.send_message, 'date', run_date=instance.get_date_time, args=[
        ...
        instance.get_sms_content], timezone=pytz.timezone(TIME_ZONE), id=instance.get_scheduler_id)


@receiver(post_delete, sender=Appointment)
def remove_scheduled_appointment(sender, instance, **kwargs):
    if SCHEDULER.get_job(instance.get_scheduler_id):  # remove the scheduled job
        SCHEDULER.remove_job(instance.get_scheduler_id)
